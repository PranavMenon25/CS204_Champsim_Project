...
static int new_set_cur;
static int new_way_cur;

pair<int, int> new_llc_find_victim(int set, int set1, PACKET *packet, uint32_t cpu, uint64_t instr_id, uint32_t set_t, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type){
  
  int tag_way1; 
  pair<int, int> new_;
  // cout<<new_set_cur<<" "<<new_way_cur<<endl;
  int flag = 0;
  if(new_set_cur < 2048 && new_way_cur < 16){
     DP ( if (warmup_complete[cpu]) {
            cout << "[" << NAME << "] " << __func__ << " instr_id: " << instr_id << " invalid set: " << set << " way: " << way;
            cout << hex << " address: " << (full_addr>>LOG2_BLOCK_SIZE) << " victim address: " << block[set][way].address << " data: " << block[set][way].data;
            cout << dec << " lru: " << block[set][way].lru << endl; });

    new_.first = new_set_cur;
    new_.second = new_way_cur;
    new_way_cur++;
    if(new_way_cur == 16 && new_set_cur < 2047){
      new_set_cur++;
      new_way_cur = 0;
    }
    for(tag_way1 = 0; tag_way1<32; tag_way1++){
      if(tag_directory_new::tag_direc[set1][tag_way1].second.second == false){
        break;
      }
    }
    if(tag_way1<32){
      tag_directory_new::tag_direc[set1][tag_way1].first = packet->address;
      // cout<<"tag _ address"<<tag_directory_new::tag_direc[set1][tag_way1].first<<endl;
      tag_directory_new::tag_direc[set1][tag_way1].second.second = true;
      tag_directory_new::tag_direc[set1][tag_way1].second.first = (16*new_.first) + new_.second;
      // cout<<tag_directory_new::tag_direc[set][tag_way1].second.second<<endl;
      // cout<<set<<" "<<tag_way1<<endl;
    }
      ...
      while (1)
      {
        // cout<<"*";
          for (int i=0; i<LLC_WAY; i++){
              if (tag_directory_new::srrip[set][i] == max_srrip){
                tag_way1 = i;
                flag = 1;
                break;
              }
    ...
    for(tag_way1 = 0; tag_way1<24; tag_way1++){
      if(tag_directory_new::tag_direc[set1][tag_way1].second.second == false){
        break;
      }
    }
    ...
    if(tag_way1 < 24){
      ...
      std::mt19937 rng(rd());
      std::uniform_int_distribution<int> dist1(0, 2047);
      
      // Generate and print the first random number between 1 and 100
      int random_number1 = dist1(rng);
      
      // Define the second distribution for integers between 200 and 300
      uniform_int_distribution<int> dist2(0, 15);
      int random_number2 = dist2(rng);
      new_.first = random_number1;
      new_.second = random_number2;
      flag = 1;
      tag_directory_new::tag_direc[set1][tag_way1].first = packet->address;
      tag_directory_new::tag_direc[set1][tag_way1].second.second = true;
      tag_directory_new::tag_direc[set1][tag_way1].second.first = (16*new_.first) + new_.second;
  ...
  if(flag == 0){
    std::random_device rd;
    std::mt19937 rng(rd());
    std::uniform_int_distribution<int> dist1(0, 31);
    
    // Generate and print the first random number between 1 and 100
    int random_number1 = dist1(rng);

    tag_way1 = random_number1;
    tag_directory_new::tag_direc[set1][tag_way1].first = packet->address;
    tag_directory_new::tag_direc[set1][tag_way1].second.second = true;
    
    int old = tag_directory_new::tag_direc[set1][tag_way1].second.first;
    new_.second =  old%16;
    new_.first = (int)old/16;
    tag_directory_new::tag_direc[set1][tag_way1].second.first = (16*new_.first) + new_.second;
  }
  // cout<<new_.first<<" "<<new_.second<<endl;
  return new_;
}
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        // find victim
        uint32_t set1 = get_set(MSHR.entry[mshr_index].address), way;

        uint32_t set = set1;
        if (cache_type == IS_LLC) {
            Added_class::set_acces[set1]++;
            uint32_t set2;
            pair<int, int> data;
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
            // cout<<MSHR.entry[mshr_index].full_addr<<" "<<MSHR.entry[mshr_index].address<<endl;
            data = new_llc_find_victim(set, set1, &MSHR.entry[mshr_index], fill_cpu, MSHR.entry[mshr_index].instr_id, set2, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
            set = data.first;
            way = data.second;
        ...
        if ((cache_type == IS_LLC) && (way == LLC_WAY)) { // this is a bypass that does not fill the LLC
              cout << "MSHR Entry = " << MSHR.entry[mshr_index].type <<endl;
			...
			if((WQ.entry[index].fill_l1d) && (MSHR.entry[mshr_index].fill_l1d != 1))
			  {
                ...
                // find victim
                uint32_t set1 = get_set(WQ.entry[index].address), way;
                uint32_t set = set1;
                uint32_t set2;
                if (cache_type == IS_LLC) {
                    ...
                    pair<int, int> data;
                    // way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
                    data = new_llc_find_victim(set, set1, &WQ.entry[index], writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type);
                    set = data.first;
                    way = data.second;
                }
                else
      ...
      if (read_cpu == NUM_CPUS)
		...
		if (RQ.entry[index].type == LOAD) {
			...
			cpu = 0;
                ...
                // cout<<sim_access[read_cpu][RQ.entry[index].type];
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    block[set][way].new_data = packet->data;
    block[set][way].full_addr = packet->full_addr;
    block[set][way].new_rev_add_row = set;
    block[set][way].new_rev_add_way = way;      
...
int CACHE::check_hit(PACKET *packet)
{
  ...
  if(cache_type == IS_LLC){
    ...
    if(cache_type == IS_LLC && warmup_complete[cpu]){
      for(final_way = 0; final_way<32 && tag_directory_new::tag_direc[set][final_way].second.second == true; final_way++){
        if(tag_directory_new::tag_direc[set][final_way].first == packet->address){
          // pos++;
          // cout<<pos<<endl;
          match_way = final_way;
    ...
    // hit
    // for (uint32_t way=0; way<32; way++) {
    //     if (tag_directory_new::tag_direc[set][way].second.second && (tag_directory_new::tag_direc[set][way].first == focus)) {
    //       // cout<<"matched way = "<<way<<"set = "<<set<<endl;
    //         match_way = way;
    //         // cout<<block[set][way].valid<<endl;
    //         // cout<<set<<" "<<way;
    //         DP ( if (warmup_complete[packet->cpu]) {
    ...
    if(match_way>-1){
      tag_directory_new::srrip[set][0] = max_srrip-1;
    }

    else{
      tag_directory_new::srrip[set][match_way] = 0;
    }
    return match_way;
}
