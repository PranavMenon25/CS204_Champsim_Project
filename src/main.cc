...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout<<"Enter print_roi_status"<<pt<<endl;
    pt++;
...
{
    cout<<"Enter print_branch_stats"<<pt<<endl;
    pt++;
...
{
    cout<<"print_dran_status"<<pt<<endl;
    pt++;
...
std::vector<int> Added_class::set_acces;
std::map<int,int> Added_class::freq_map;
std::vector<int> Added_class::set_cur_length;
std::vector<std::vector<std::pair<uint64_t, pair<int, bool>>>> tag_directory_new::tag_direc;
std::vector<vector<int>> tag_directory_new::srrip;
...
int main(int argc, char** argv)
{
    for(int i = 0; i<2048; i++){
        Added_class::set_acces.push_back(0);
        Added_class::set_cur_length.push_back(0);
    }
    ...
    for(int i = 0; i<2048; i++){
        ...
        tag_directory_new::srrip.push_back(pv);
    }

    for(int i = 0; i<2048; i++){
        std::vector<std::pair<uint64_t, pair<int, bool>>> pv;
        for(int i = 0; i<32; i++){
            pair<uint64_t, pair<int, bool>> open;
            pair<int, bool> open2;
            open2.first = 0;
            open2.second = false;
            open.first = 0;
        ...
        tag_directory_new::tag_direc.push_back(pv);
    }
	      ...
	      if ((ooo_cpu[i].IFETCH_BUFFER.occupancy < ooo_cpu[i].IFETCH_BUFFER.SIZE) && (ooo_cpu[i].fetch_stall == 0))
		{
		  ooo_cpu[i].read_from_trace();
            ...
            if (show_heartbeat && (ooo_cpu[i].num_retired >= ooo_cpu[i].next_print_instruction)) {
                ...
                cout << " heartIPCbeat : " << heartbeat_ipc << " cumulative IPC: " << cumulative_ipc; 
                cout << " (Simulation time: " << elapsed_hour << " hr " << elapsed_minute << " min " << elapsed_second << " sec) " << endl;
    ...
    int total = 0;
    // vector<pair<int,int>> Old_pair(2048,{0,0});
    for(int i = 0; i<2048; i++){
    //     Old_pair[i].second=Added_class::set_acces[i];
    //     Old_pair[i].first=i;
        total += Added_class::set_acces[i];
        cout<<"Total access = "<<total;
    }
    // sort(Old_pair.begin(),Old_pair.end(),compareSecond);
    // for(auto it:Old_pair)
    // {
